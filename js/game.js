...
const CONFIG = {
    ...
    MAIN_BOSS_INTERVAL: 120000,
    MINI_BOSS_SONIC_SCALE: 1.5,
};
...
const Game = {
    ...
    init() {
        ...
        this.setupInput();
        this.highScore = parseInt(localStorage.getItem('neorobo_highscore') || '0');
    },
    ...
    spawnBoss(isMainBoss = false) {
        ...
        const bw = 80 * scale;
        const bh = 105 * scale;
        const by = this.mainPlatTop + CONFIG.GROUND_ON_MAIN - bh - 50;
        const availableRobots = ASSETS.robots.filter((_, i) => i !== this.selectedRobot);
    ...
    update(dt) {
        ...
        // Track player movement for boss facing logic
        ...
        const playerMoved = p.x !== prevX || p.y !== prevY || p.vx !== 0 || p.vy !== 0;
        ...
        // Timer-based boss spawning
        this.miniBossTimer += dt;
        this.mainBossTimer += dt;
        if (this.miniBossTimer >= CONFIG.MINI_BOSS_INTERVAL) {
            this.miniBossTimer -= CONFIG.MINI_BOSS_INTERVAL;
            this.spawnBoss(false);
        }
        if (this.mainBossTimer >= CONFIG.MAIN_BOSS_INTERVAL) {
            this.mainBossTimer -= CONFIG.MAIN_BOSS_INTERVAL;
            this.spawnBoss(true);
        }
        ...
        for (let bi = this.bosses.length - 1; bi >= 0; bi--) {
            ...
            // Facing logic: don't auto-face if directly below and player hasn't moved
            const isBelowPlayer = b.y + b.h > p.y + p.h * 0.5 && Math.abs((b.x + b.w/2) - (p.x + p.w/2)) < 400;
            if (!isBelowPlayer || playerMoved) {
            ...
            // All bosses shoot sonic waves
            b.sonicTimer += dt;
            if (b.sonicTimer >= CONFIG.BOSS_SONIC_INTERVAL) {
                b.sonicTimer = 0;
                const bDir = b.dir;
                const waveW = b.isMainBoss ? CONFIG.BOSS_SONIC_WIDTH : CONFIG.SONIC_WAVE_WIDTH * CONFIG.MINI_BOSS_SONIC_SCALE;
                const waveH = b.isMainBoss ? CONFIG.BOSS_SONIC_HEIGHT : CONFIG.SONIC_WAVE_HEIGHT * CONFIG.MINI_BOSS_SONIC_SCALE;
    ...
    gameOver() {
        ...
        if (this.points > this.highScore) {
            ...
            localStorage.setItem('neorobo_highscore', this.highScore.toString());
        }
