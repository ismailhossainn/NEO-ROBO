...
const Game = {
    ...
    playerLevel: 1,
    sonicDamage: 1,
    lastStandardLevelUpAt: 0,
    lastMajorLevelUpAt: 0,
    levelUpAnim: null,
    worldEndX: 0,
    segmentIndex: 0,
    keys: {},
    ...
    generateInitialWorld() {
        ...
        this.segmentIndex = 0;
        this.bosses = [];
        this.largeBossActive = false;
        this.bossesDefeated = 0;
        this.bossSonicWaves = [];
        this.miniBossTimer = 0;
        this.largeBossTimer = 0;
        this.lastStandardLevelUpAt = 0;
        this.lastMajorLevelUpAt = 0;
    ...
    spawnBoss(isMainBoss = false) {
        if (isMainBoss && this.largeBossActive) return;
        ...
        if (isMainBoss) this.largeBossActive = true;
    ...
    update(dt) {
        ...
        for (let i = this.bosses.length - 1; i >= 0; i--) {
            ...
            if (!b.alive) {
                if (b.isMainBoss) this.largeBossActive = false;
        ...
        // Mini boss spawn: every 60 seconds
        ...
        // Large boss spawn: every 150 seconds, but only if no large boss is active
        this.largeBossTimer += dt;
        if (this.largeBossTimer >= 150000) {
            this.largeBossTimer = 0;
            if (!this.largeBossActive) {
                this.spawnBoss(true);
            }
        }
        ...
        // Standard level up: every 1,500 points
        const nextStandard = Math.floor(this.points / 1500) * 1500;
        if (nextStandard > 0 && nextStandard > this.lastStandardLevelUpAt) {
            this.lastStandardLevelUpAt = nextStandard;
            this.playerLevel++;
            this.sonicDamage = Math.pow(2, this.playerLevel - 1);
            this.levelUpAnim = { timer: 0, duration: 150 };
            AudioManager.play('victory');
        }

        // Major level up: every 2,500 points
        const nextMajor = Math.floor(this.points / 2500) * 2500;
        if (nextMajor > 0 && nextMajor > this.lastMajorLevelUpAt) {
            this.lastMajorLevelUpAt = nextMajor;
            this.playerLevel++;
            this.sonicDamage = Math.pow(2, this.playerLevel - 1);
            this.levelUpAnim = { timer: 0, duration: 150 };
            AudioManager.play('victory');
    ...
    loop(timestamp) {
        ...
        const FIXED_DT = 1000 / 60; // 16.666... ms for 60 FPS
