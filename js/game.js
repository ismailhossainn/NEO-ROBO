...
const Game = {
    ...
    spawnBoss(isMainBoss = false) {
        ...
        // Find a valid main platform to spawn on
        let spawnX = this.cameraX + this.vw + 80;
        let spawnY = this.mainPlatTop + CONFIG.GROUND_ON_MAIN - bh;
        
        for (const plat of this.platforms) {
            if (plat.type === 'main' && spawnX >= plat.x && spawnX <= plat.x + plat.w) {
                spawnY = plat.y + CONFIG.GROUND_ON_MAIN - bh;
                break;
            }
        }
        ...
        if (this.player) {
            b.facingRight = this.player.x > b.x;
        }
    ...
    update(dt) {
        ...
        for (const b of this.bosses) {
            ...
            // Boss physics - gravity
            b.vy += CONFIG.GRAVITY;
            if (b.vy > CONFIG.MAX_FALL_SPEED) b.vy = CONFIG.MAX_FALL_SPEED;
            
            // AI: move toward player horizontally
            const dx = (p.x + p.w/2) - (b.x + b.w/2);
            if (Math.abs(dx) > 10) {
                b.vx = dx > 0 ? b.speed : -b.speed;
            } else {
                b.vx = 0;
            }
            ...
            // Platform collision (same as player)
            b.onGround = false;
            ...
            for (const plat of this.platforms) {
                ...
                if (b.vy >= 0 &&
                    ...
                    b.y + b.h >= surfaceY - 6 && b.y + b.h <= surfaceY + b.vy + 12) {
                    b.y = landY;
                    b.vy = 0;
                    b.onGround = true;
                    b.jumps = 0;
            ...
            // Gap detection - jump across gaps like the player
            if (b.onGround && Math.abs(b.vx) > 0.1 && b.currentPlatform) {
                const moveDir = b.vx > 0 ? 1 : -1;
                const checkX = b.x + (moveDir > 0 ? b.w + 20 : -20);
                const checkY = b.y + b.h + 15;
                const currentSurfaceY = b.currentPlatform.y + (b.currentPlatform.tier === 1 ? CONFIG.GROUND_ON_MAIN : 
                                       b.currentPlatform.tier === 2 ? CONFIG.GROUND_ON_STATIC : CONFIG.GROUND_ON_MOVING);
                
                let groundAhead = false;
                for (const plat of this.platforms) {
                    if (plat === b.currentPlatform) continue;
                    let groundOff;
                    if (plat.tier === 1) groundOff = CONFIG.GROUND_ON_MAIN;
                    else if (plat.tier === 2) groundOff = CONFIG.GROUND_ON_STATIC;
                    else groundOff = CONFIG.GROUND_ON_MOVING;
                    ...
                    if (checkX >= plat.x && checkX <= plat.x + plat.w &&
                        Math.abs(surfaceY - currentSurfaceY) < 40) {
                        groundAhead = true;
                        break;
                    }
                }
                ...
                if (!groundAhead) {
                    b.vy = CONFIG.JUMP_FORCE * 0.9;
                    b.onGround = false;
                    b.jumps = 1;
                }
            }
            
            // Remove fallen bosses
            if (b.y > this.vh + 300) {
                b.alive = false;
                continue;
            }
            ...
            // Facing logic
            if (b.y > p.y) {
                // Boss is below player, face the direction it is coming from (movement direction)
                b.facingRight = b.vx >= 0;
            } else {
                // Face the player whenever the player moves left or right (and when stationary)
                b.facingRight = p.x + p.w/2 > b.x + b.w/2;
            }
            
            // Hit flash
            if (b.hitFlash > 0) b.hitFlash--;
            
            // Sonic wave attack (all bosses, including mini bosses)
            b.sonicTimer += dt;
            if (b.sonicTimer >= CONFIG.BOSS_SONIC_INTERVAL) {
                b.sonicTimer = 0;
                const bDir = b.facingRight ? 1 : -1;
                // Mini boss waves are 1.5x larger than player's energy wave
                const waveW = b.isMainBoss ? CONFIG.BOSS_SONIC_WIDTH : CONFIG.SONIC_WAVE_WIDTH * 1.5;
                const waveH = b.isMainBoss ? CONFIG.BOSS_SONIC_HEIGHT : CONFIG.SONIC_WAVE_HEIGHT * 1.5;
                this.bossSonicWaves.push({
                    x: b.x + (bDir > 0 ? b.w : -waveW),
                    y: b.y + b.h * 0.3,
                    w: waveW, h: waveH,
        ...
        // Timer-based mini boss spawn (every 40 seconds)
        this.bossSpawnTimer += dt;
        if (this.bossSpawnTimer >= 40000) {
            this.bossSpawnTimer -= 40000;
            this.spawnBoss(false);
        }

        // Main boss spawn (every 2000 points)
        if (this.points > 0) {
        ...
        // Level up check: every 2000 points, repeating continually
        ...
        this.bosses = this.bosses.filter(b => b.alive && b.y < this.vh + 400 && b.x + b.w > cleanX);
    },
