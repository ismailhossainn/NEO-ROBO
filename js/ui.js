...
const UI = {
    ...
    setupStartScreen() {
        ...
        // === Mandatory ONE-TIME registration (per device) ===
        // If a pilot name is already saved on this device, skip the
        // registration screen entirely and go straight to robot selection.
        const goNextFromStart = () => {
            AudioManager.init();
            AudioManager.play('click');

            // Re-check persistent storage in case it was registered earlier
            let saved = null;
            try { saved = PlayerRegistry.loadCurrent(); } catch (e) { saved = null; }
            // Make sure the saved name still exists in the pilot list
            if (saved && PlayerRegistry.exists(saved)) {
                this.showScreen('selection');
                this.initRobotSelection();
            } else {
                // No registered pilot yet on this device → force registration
    ...
    showGameOver(points, highScore) {
        ...
        // Session-only high score (NOT persisted between sessions)
    ...
    setupGameOverScreen() {
        ...
        const home = () => {
            ...
            // The pilot name is PERMANENT on this device — do NOT clear it.
            // Next time the player presses START, they'll skip registration.
            Game.state = 'start';
            this.showScreen('start');
        };
