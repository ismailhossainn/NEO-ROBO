...
const CONFIG = {
    ...
    BOSS_SONIC_LIFE: 180,
    STANDARD_LEVEL_UP_POINTS: 1500,
    MAJOR_LEVEL_UP_POINTS: 2500,
    MINI_BOSS_INTERVAL: 30000,
    LARGE_BOSS_INTERVAL: 90000,
};
...
const Game = {
    ...
    generateInitialWorld() {
        ...
        this.lastLargeBossTime = 0;
        this.lastStandardLevelUp = 0;
        this.lastMajorLevelUp = 0;
    ...
    update(dt) {
        ...
        if (!this.bossActive) {
            // Large boss every 1 minute 30 seconds
            if (this.gameTimer >= this.lastLargeBossTime + CONFIG.LARGE_BOSS_INTERVAL) {
                this.lastLargeBossTime = this.gameTimer;
                this.spawnBoss(true);
            }
            // Mini boss every 30 seconds
            else if (this.gameTimer >= this.lastMiniBossTime + CONFIG.MINI_BOSS_INTERVAL) {
        ...
        // Standard level up every 1500 points
        if (this.points >= this.lastStandardLevelUp + CONFIG.STANDARD_LEVEL_UP_POINTS) {
            this.lastStandardLevelUp += CONFIG.STANDARD_LEVEL_UP_POINTS;
            this.playerLevel++;
        ...
        // Major level up every 2500 points
        if (this.points >= this.lastMajorLevelUp + CONFIG.MAJOR_LEVEL_UP_POINTS) {
            this.lastMajorLevelUp += CONFIG.MAJOR_LEVEL_UP_POINTS;
            this.playerLevel++;
            this.sonicDamage = Math.pow(2, this.playerLevel - 1);
            this.levelUpAnim = { timer: 0, duration: 150 };
            AudioManager.play('victory');
    ...
    loop(timestamp) {
        ...
        const EPSILON = 0.001;
        this.accumulator += dt;
        if (this.accumulator > FIXED_DT * 3) this.accumulator = FIXED_DT * 3;

        while (this.accumulator >= FIXED_DT - EPSILON) {
