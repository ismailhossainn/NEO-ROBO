...
const CONFIG = {
    ...
    BOSS_SONIC_LIFE: 180,
    // === Fixed 60 FPS update ===
    FIXED_TIMESTEP_MS: 1000 / 60,   // 16.6667ms per logic tick
    MAX_UPDATES_PER_FRAME: 5,        // safety cap to avoid spiral of death
    // === Mini boss time-based spawn ===
    MINI_BOSS_INTERVAL_MS: 40000,    // spawn a mini boss every 40 seconds
    // === Boss physics (jump-across-gaps) ===
    BOSS_GRAVITY: 0.55,
    BOSS_JUMP_FORCE: -16.2,
    BOSS_MAX_FALL: 14,
    BOSS_GAP_LOOKAHEAD: 60,          // how far ahead to look for ground
    BOSS_JUMP_COOLDOWN: 18,          // frames between auto-jumps
    // === Mini-boss sonic wave (1.5x player's wave, same color as main boss) ===
    MINI_BOSS_SONIC_WIDTH: 38,       // 25 (player) * 1.5
    MINI_BOSS_SONIC_HEIGHT: 38,
    MINI_BOSS_SONIC_SPEED: 5.5,
    MINI_BOSS_SONIC_INTERVAL: 3500,  // ms between mini boss shots
    MINI_BOSS_SONIC_DAMAGE: 10,
    MINI_BOSS_SONIC_LIFE: 180,
};
...
const Game = {
    ...
    sonicWaves: [],
    particles: [],
    // Bosses now stored as an array so multiple bosses can stack on screen
    bosses: [],
    bossSpawnTimer: 0,         // ms accumulator for time-based mini-boss spawn
    lastMainBossAt: 0,
    bossesDefeated: 0,
    bossSonicWaves: [],
    points: 0,
    ...
    spawnBoss(isMainBoss = false) {
        ...
        const bw = 80 * scale;
        const bh = 105 * scale;
        // Spawn on top of the main platform so the boss starts grounded
        ...
        // Decide spawn side: prefer the side ahead of the player so the boss has to chase them
        const p = this.player;
        const spawnRight = !p || (Math.random() < 0.5 ? p.x > this.cameraX + this.vw * 0.4 : true);
        const spawnX = spawnRight
            ? this.cameraX + this.vw + 80
            : Math.max(0, this.cameraX - bw - 80);
        const boss = {
            x: spawnX,
            y: by, w: bw, h: bh,
            vx: 0, vy: 0,
            ...
            // All bosses (mini AND main) now shoot energy waves.
            // Mini bosses fire on a slightly different cadence and at smaller size.
            sonicTimer: 0
        };
    ...
    // Returns true if there's a main-platform ground tile at the given world point.
    // Used by boss AI to decide whether to jump across a gap.
    hasMainGroundAt(worldX, worldY) {
        for (const plat of this.platforms) {
            if (plat.type !== 'main') continue;
            let groundOff = CONFIG.GROUND_ON_MAIN;
            const surfaceY = plat.y + groundOff;
            if (worldX >= plat.x && worldX <= plat.x + plat.w &&
                worldY >= surfaceY - 4 && worldY <= surfaceY + 60) {
                return true;
            }
        }
        return false;
    ...
    update(dt) {
        ...
        // ============ BOSS UPDATE (multiple bosses, full physics, gap-jumping) ============
        for (let bi = this.bosses.length - 1; bi >= 0; bi--) {
            const b = this.bosses[bi];
            if (!b.alive) continue;

            // -- Facing & horizontal movement --
            // Always face the player (this naturally handles "turn to face player whenever
            // player moves left or right" and "when below player face the direction it's
            // coming from" since the boss is always travelling toward the player).
            ...
            // -- Gravity --
            b.vy += CONFIG.BOSS_GRAVITY;
            if (b.vy > CONFIG.BOSS_MAX_FALL) b.vy = CONFIG.BOSS_MAX_FALL;

            // -- Auto-jump across gaps (similar to how the player jumps) --
            if (b.jumpCooldown > 0) b.jumpCooldown--;
            if (b.onGround && b.jumpCooldown === 0) {
                // Look slightly ahead in the boss's movement direction for ground.
                const lookX = b.dir > 0
                    ? b.x + b.w + CONFIG.BOSS_GAP_LOOKAHEAD
                    : b.x - CONFIG.BOSS_GAP_LOOKAHEAD;
                const lookY = b.y + b.h + 8;
                const groundAhead = this.hasMainGroundAt(lookX, lookY);
                // Also jump if the player is above (so the boss tries to chase upward)
                const playerAbove = (p.y + p.h) < (b.y - 30);
                if (!groundAhead || playerAbove) {
                    b.vy = CONFIG.BOSS_JUMP_FORCE;
                    b.onGround = false;
                    b.jumpCooldown = CONFIG.BOSS_JUMP_COOLDOWN;
                }
            }
            ...
            // -- Platform landing (same shape as the player landing check) --
            b.onGround = false;
            b.currentPlatform = null;
            for (const plat of this.platforms) {
                if (plat.type !== 'main') continue; // bosses only stand on main ground
                let groundOff = CONFIG.GROUND_ON_MAIN;
                const surfaceY = plat.y + groundOff;
                const landY = surfaceY - b.h;
                if (b.vy >= 0 &&
                    b.x + b.w > plat.x + 8 && b.x < plat.x + plat.w - 8 &&
                    b.y + b.h >= surfaceY - 6 && b.y + b.h <= surfaceY + b.vy + 14) {
                    b.y = landY;
                    b.vy = 0;
                    b.onGround = true;
                    b.currentPlatform = plat;
                    break;
                }
            }

            // Boss fell off the world — remove it
            if (b.y > this.vh + 200) {
                b.alive = false;
                this.bosses.splice(bi, 1);
                continue;
            }
            ...
            // -- Sonic wave attack --
            // Main bosses use the original big wave. Mini bosses use a 1.5x-player-sized
            // wave with the same color palette as the main boss's wave.
            b.sonicTimer += dt;
            const fireInterval = b.isMainBoss ? CONFIG.BOSS_SONIC_INTERVAL : CONFIG.MINI_BOSS_SONIC_INTERVAL;
            if (b.sonicTimer >= fireInterval) {
                b.sonicTimer = 0;
                const bDir = b.dir;
                if (b.isMainBoss) {
                    ...
                    this.bossSonicWaves.push({
                        x: b.x + (bDir > 0 ? b.w : -CONFIG.MINI_BOSS_SONIC_WIDTH),
                        y: b.y + b.h * 0.35,
                        w: CONFIG.MINI_BOSS_SONIC_WIDTH, h: CONFIG.MINI_BOSS_SONIC_HEIGHT,
        ...
        // Update boss sonic waves (now carry their own damage so mini/main can differ)
        for (let i = this.bossSonicWaves.length - 1; i >= 0; i--) {
            ...
            if (!p.invincible && this.rectsOverlap(bsw, p)) {
                const dmg = (bsw.damage != null) ? bsw.damage : CONFIG.BOSS_SONIC_DAMAGE;
                this.playerHit(dmg);
        ...
        // ============ BOSS SPAWN LOGIC ============
        // Bosses now stack — spawns happen on their own timers regardless of
        // whether previous bosses are still alive.
        //
        // Mini boss: TIME-BASED every 40 seconds.
        // Main boss: POINTS-BASED every 2000 points (unchanged).
        this.bossSpawnTimer += dt;
        while (this.bossSpawnTimer >= CONFIG.MINI_BOSS_INTERVAL_MS) {
            this.bossSpawnTimer -= CONFIG.MINI_BOSS_INTERVAL_MS;
        ...
        for (let i = this.sonicWaves.length - 1; i >= 0; i--) {
            ...
            if (!this.sonicWaves[i]) continue;
            // Player wave vs any of the active bosses
            let waveHitBoss = false;
            for (let bi = this.bosses.length - 1; bi >= 0; bi--) {
                const bb = this.bosses[bi];
                if (!bb.alive) continue;
                if (this.rectsOverlap(w, bb)) {
                    bb.health -= this.sonicDamage; bb.hitFlash = 10;
                    this.spawnParticles(bb.x + bb.w/2, bb.y, '#b400ff', 10);
                    AudioManager.play('click');
                    waveHitBoss = true;
                    if (bb.health <= 0) {
                        bb.alive = false;
                        this.bossesDefeated++;
                        this.points += bb.isMainBoss ? 500 : 200;
                        this.spawnParticles(bb.x + bb.w/2, bb.y + bb.h/2, '#ffcc00', bb.isMainBoss ? 35 : 20);
                        AudioManager.play('victory');
                        this.bosses.splice(bi, 1);
                    }
                    break;
    ...
    render() {
        ...
        for (const bb of this.bosses) { if (bb.alive) this.drawBoss(ctx, bb); }
    ...
    // ------------------------------------------------------------------------
    // We render every animation frame (so motion stays smooth on high-refresh
    // monitors), but the physics/AI `update()` step runs at a FIXED 60 Hz tick.
    // This guarantees identical gameplay speed on 60Hz, 120Hz, 144Hz, 240Hz, etc.
    // ========================================================================
    loop(timestamp) {
        // Real wall-clock delta since last frame
        let frameDelta = timestamp - this.lastTime;
        this.lastTime = timestamp;

        // Clamp huge gaps (e.g. tab was backgrounded) so we don't try to catch
        // up by simulating thousands of ticks.
        if (frameDelta > 250) frameDelta = 250;

        this._fixedAccumulator = (this._fixedAccumulator || 0) + frameDelta;

        const step = CONFIG.FIXED_TIMESTEP_MS; // 16.6667ms == 60 FPS
        let updates = 0;
        while (this._fixedAccumulator >= step && updates < CONFIG.MAX_UPDATES_PER_FRAME) {
            // Always feed the SAME fixed dt to update() so all timers and
            // per-tick motion are frame-rate independent.
            this.deltaTime = step;
            this.update(step);
            this._fixedAccumulator -= step;
            updates++;
        }
        // If we still have a huge backlog, drop it to avoid the spiral of death.
        if (this._fixedAccumulator > step * CONFIG.MAX_UPDATES_PER_FRAME) {
            this._fixedAccumulator = 0;
        }
