...
const Game = {
    ...
    update(dt) {
        ...
        // Update all bosses (stackable, physics-based, gap-jumping)
        for (let bi = this.bosses.length - 1; bi >= 0; bi--) {
            const b = this.bosses[bi];
            if (!b.alive) continue;

            // Facing logic: don't auto-face when directly below unless player moves
            const isBelowPlayer = b.y + b.h > p.y + p.h &&
                                  b.x + b.w > p.x && b.x < p.x + p.w;
            const playerIsMoving = Math.abs(p.vx) > 0.1 || Math.abs(p.vy) > 0.1 || !p.onGround;

            if (!isBelowPlayer || playerIsMoving) {
                if (b.x + b.w/2 > p.x + p.w/2) b.dir = -1;
                else b.dir = 1;
            }

            // Gap detection & jumping
            if (b.onGround) {
                const lookAhead = b.dir > 0 ? b.x + b.w + 15 : b.x - 15;
                let groundAhead = false;
                for (const plat of this.platforms) {
                    if (plat.type === 'main' && lookAhead >= plat.x && lookAhead <= plat.x + plat.w) {
                        groundAhead = true;
                        break;
                    }
                }
                if (!groundAhead) {
                    ...
                    const startX = b.dir > 0 ? lookAhead : lookAhead - searchRange;
                    const endX = b.dir > 0 ? lookAhead + searchRange : lookAhead;
                    let canLand = false;
                    for (const plat of this.platforms) {
                        if (plat.type === 'main' && plat.x < endX && plat.x + plat.w > startX) {
                            canLand = true;
                            break;
                        }
                    }
            ...
            // Boss platform collision (main platforms only)
            b.onGround = false;
            b.currentPlatform = null;
            for (const plat of this.platforms) {
                if (plat.type !== 'main') continue;
                const surfaceY = plat.y + CONFIG.GROUND_ON_MAIN;
        ...
        // Mini boss spawn every 40 seconds (timer based, stacks)
        if (this.gameTimer >= this.lastMiniBossTime + CONFIG.MINI_BOSS_INTERVAL) {
            this.lastMiniBossTime += CONFIG.MINI_BOSS_INTERVAL;
            this.spawnBoss(false);
        }

        // Main boss spawn by points (every 2000 points, stacks)
        if (this.points > 0) {
        ...
        for (let i = this.sonicWaves.length - 1; i >= 0; i--) {
            ...
            for (const b of this.bosses) {
                if (b.alive && this.rectsOverlap(w, b)) {
        ...
        this.bosses = this.bosses.filter(b => b.alive && b.x + b.w > cleanX);
    },
    ...
    loop(timestamp) {
        ...
        // Prevent huge delta spikes (e.g. tab switching)
        if (deltaTime > 200) deltaTime = 200;

        const TARGET_DT = 1000 / 60; // Fixed 60 FPS timestep

        if (this.state === 'playing') {
            this.accumulator += deltaTime;
            // Process fixed timestep updates
            while (this.accumulator >= TARGET_DT) {
                this.update(TARGET_DT);
                this.accumulator -= TARGET_DT;
            }
        } else {
            this.accumulator = 0;
