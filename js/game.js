...
const Game = {
    ...
    init() {
        ...
        try {
            const hs = parseInt(localStorage.getItem('neoRoboHighScore') || '0', 10);
    ...
    spawnBoss(isMainBoss = false) {
        ...
        // Spawn from either side of camera (randomized so stacked bosses don't overlap badly)
        const fromRight = Math.random() < 0.5 ? 1 : -1;
        const spawnX = fromRight > 0
            ? this.cameraX + this.vw + 80 + Math.random() * 200
            : this.cameraX - bw - 80 - Math.random() * 200;
        const availableRobots = ASSETS.robots.filter((_, i) => i !== this.selectedRobot);
    ...
    update(dt) {
        ...
        if (this.points > this.highScore) {
            ...
            try { localStorage.setItem('neoRoboHighScore', String(this.highScore)); } catch (e) {}
        }
    },
