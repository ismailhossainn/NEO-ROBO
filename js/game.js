...
const CONFIG = {
    ...
    BOSS_SONIC_LIFE: 180,
    // Mini-boss wave = 1.5x player's wave (player SONIC_WAVE_WIDTH/HEIGHT = 25 → 37.5)
...
const Game = {
    ...
    sonicWaves: [],
    particles: [],
    bosses: [],                 // multiple bosses can be active simultaneously
    miniBossSpawnTimer: 0,      // ms since last mini-boss spawn
    mainBossSpawnTimer: 0,      // ms since last main-boss spawn
    bossesDefeated: 0,
    bossSonicWaves: [],
    playerHasMoved: false,      // becomes true on first player horizontal input; latches per-frame for facing-freeze logic
    points: 0,
    highScore: 0,
    gameTimer: 0,
    health: 100,
    ...
    lastTime: 0,
    deltaTime: 0,
    accumulator: 0,             // for fixed-timestep loop
    animFrame: 0,
    mainPlatTop: 0,
    mainPlatBot: 0,
    ...
    init() {
        ...
        this.setupInput();
        // High score is SESSION-ONLY — not persisted between sessions.
        // (Per-pilot scores are still saved separately via PlayerRegistry
        //  for the ranking scoreboard.)
        this.highScore = 0;
    },
    ...
    spawnBoss(isMainBoss = false) {
        ...
        const boss = {
            ...
            baseFloatY: baseFloatY,    // resting float Y above main platform
            gapState: 'normal',        // 'normal' | 'rising' | 'crossing' | 'descending'
            // Both mini-boss and main-boss now shoot energy waves
            sonicTimer: 0
        };
    ...
    // Helper: check whether there is solid ground (main platform) directly under x
    hasGroundUnder(x) {
    ...
    update(dt) {
        ...
        p.vx = 0;
        // Track whether the player issued any horizontal input THIS frame (used to release boss facing-freeze)
        ...
        // ===== BOSSES (multiple can be active simultaneously) =====
        for (const b of this.bosses) {
            if (!b.alive) continue;

            // ----- FACING / HORIZONTAL MOVEMENT -----
            // Detect: is the player horizontally above this boss (within boss footprint)?
            const playerAboveBoss =
                p.x + p.w > b.x && p.x < b.x + b.w &&
                p.y + p.h <= b.y;  // player is at or above boss's top

            // Freeze facing/movement direction when player is directly above AND player hasn't moved this frame.
            // Boss continues facing the direction it was moving from until player makes a movement.
            if (playerAboveBoss && !this.playerHasMoved) {
                // Keep current dir, but still inch in that direction so the boss "continues facing" it
                b.x += b.speed * b.dir;
            } else {
                // Normal pursuit: face & move toward the player
                if (b.x + b.w / 2 > p.x + p.w / 2) { b.dir = -1; b.x -= b.speed; }
                else                                { b.dir =  1; b.x += b.speed; }
            }

            // ----- VERTICAL: float above ground, lift over gaps -----
            const surfaceY = this.mainPlatTop + CONFIG.GROUND_ON_MAIN;
            ...
            // Sample several points along the boss's footprint to decide if any solid ground exists
            ...
            // ----- ENERGY WAVE ATTACK -----
            // Both main bosses and mini bosses now fire an energy wave.
            b.sonicTimer += dt;
            ...
            if (b.sonicTimer >= interval) {
                ...
                if (b.isMainBoss) {
                    ...
                    // Mini-boss wave: SAME COLOR as main boss wave (rendered identically),
                    // size is 1.5x the PLAYER's wave (player = SONIC_WAVE_WIDTH/HEIGHT).
                    ...
                    this.bossSonicWaves.push({
                        ...
                        isMainBossWave: false  // mini-boss wave, but uses same color/style
                    });
                }
            }
        ...
        // Update boss sonic waves (both main-boss and mini-boss waves live here)
        ...
        // Mini boss every 40s, main boss every 2min. Spawns continue regardless of previous
        // bosses being defeated, and can stack (multiple bosses on screen simultaneously).
        ...
        for (let i = this.sonicWaves.length - 1; i >= 0; i--) {
            ...
            if (!this.sonicWaves[i]) continue;
            // Player sonic vs every active boss
            let hitABoss = false;
            for (const b of this.bosses) {
                if (!b.alive) continue;
                if (this.rectsOverlap(w, b)) {
                    b.health -= this.sonicDamage; b.hitFlash = 10;
        ...
        // Drop dead bosses (and bosses that have drifted very far off-screen)
        this.bosses = this.bosses.filter(b => b.alive && b.x + b.w > cleanX - 400 && b.x < this.cameraX + this.vw + 2000);

        // ===== HIGH SCORE update (live, while playing) =====
        // Session-only — no longer persisted to localStorage.
    ...
    drawBossSonicWave(ctx, bsw) {
        // Mini-boss waves use the SAME color/style as main-boss waves.
    ...
    gameOver() {
        this.state = 'gameover';
        // Final high-score commit (SESSION ONLY — not persisted)
        if (this.points > this.highScore) {
            this.highScore = this.points;
        }
        // === Save score to player scoreboard (per-player) ===
        // The per-pilot scoreboard is still saved permanently so the
        // ranking box can show all registered pilots on this device.
        try {
            if (typeof PlayerRegistry !== 'undefined' && PlayerRegistry.currentPlayer) {
                PlayerRegistry.recordScore(PlayerRegistry.currentPlayer, this.points);
            }
        } catch (e) {}
    ...
    loop(timestamp) {
        ...
        // Render at the monitor's refresh rate, but UPDATE the simulation at a fixed 60 Hz
        // so gameplay speed is identical on 60 / 120 / 144 / 240 Hz displays.
        let frameTime = timestamp - this.lastTime;
        this.lastTime = timestamp;
        // Clamp huge jumps (tab refocus, breakpoint) to prevent the "spiral of death"
        if (frameTime > 250) frameTime = 250;
        if (frameTime < 0) frameTime = 0;
        this.deltaTime = frameTime;

        this.accumulator += frameTime;
        // Cap accumulator so we never run more than ~5 catch-up updates per render
        ...
        this.render();
        if (this.state === 'playing') UI.updateHUD(this.health, this.energy, this.points, this.gameTimer, this.highScore);
