...
// ============ GAME CONFIG ============
// Height is ALWAYS 1080px. Width adapts to screen aspect ratio.
const CONFIG = {
    // Fixed height, width calculated at runtime
    DESIGN_HEIGHT: 1080,
    
    // Platform offsets (in 1080px design space)
    ...
    // *** PLAYER SIZE 1.5x ***
    PLAYER_WIDTH: 120,   // was 80
    PLAYER_HEIGHT: 158,  // was 105
    
    // *** ENEMY SIZE 1.5x ***
    ENEMY_WIDTH: 120,    // was 80
    ENEMY_HEIGHT: 158,   // was 105
    ENEMY_SPEED: 1.8,
    
    // *** FLYING ENEMY 1.5x ***
    FLYING_ENEMY_WIDTH: 82,   // was 55
    FLYING_ENEMY_HEIGHT: 82,  // was 55
    FLYING_ENEMY_RANGE: 100,
    
    // *** BOSS 2x (of original, not of 1.5x) ***
    BOSS_SCALE: 2.0,   // was 1.5
    BOSS_HEALTH: 5,
...
let PLAT_MAIN_H = 300;   // was 200 -> 1.5x
let PLAT_MAIN_W = 1920;
let PLAT_STATIC_H = 120;  // was 80 -> 1.5x
let PLAT_STATIC_W = 675;  // was 450 -> 1.5x
let PLAT_MOVING_H = 90;   // was 60 -> 1.5x
let PLAT_MOVING_W = 480;  // was 320 -> 1.5x

function computePlatformDimensions() {
    const pmImg = ImageSizes[ASSETS.platform_main];
    if (pmImg) {
        PLAT_MAIN_H = 300;  // 1.5x of 200
        PLAT_MAIN_W = PLAT_MAIN_H * (pmImg.w / pmImg.h);
    }
    const psImg = ImageSizes[ASSETS.platform_static];
    if (psImg) {
        PLAT_STATIC_H = 120;  // 1.5x of 80
        PLAT_STATIC_W = PLAT_STATIC_H * (psImg.w / psImg.h);
    }
    const pmvImg = ImageSizes[ASSETS.platform_moving];
    if (pmvImg) {
        PLAT_MOVING_H = 90;   // 1.5x of 60
...
const Game = {
    ...
    // Virtual resolution: height always 1080, width calculated from screen ratio
    vw: 1920,
    vh: 1080,
    ...
    // Dynamically calculate canvas size: height=1080, width based on screen aspect ratio
    updateCanvasSize() {
        const screenW = window.innerWidth || document.documentElement.clientWidth;
        const screenH = window.innerHeight || document.documentElement.clientHeight;
        const aspect = screenW / screenH;
        
        this.vh = CONFIG.DESIGN_HEIGHT;  // always 1080
        this.vw = Math.round(this.vh * aspect);  // width adapts to screen
        
        this.canvas.width = this.vw;
        this.canvas.height = this.vh;
        
        // Compute platform y positions
        this.mainPlatBot = this.vh - CONFIG.PLATFORM_MAIN_BOTTOM_GAP;
        this.mainPlatTop = this.mainPlatBot - PLAT_MAIN_H;
        
        // Store mapping for touch coordinates
    ...
    // ============ BOSS (2x original size) ============
    spawnBoss() {
        // Boss uses CONFIG.BOSS_SCALE (2.0) * original player size (80x105)
        const bw = 80 * CONFIG.BOSS_SCALE;   // 160px
        const bh = 105 * CONFIG.BOSS_SCALE;  // 210px
        ...
        this.boss = {
            ...
            y: by, w: bw, h: bh,
            health: CONFIG.BOSS_HEALTH, maxHealth: CONFIG.BOSS_HEALTH,
            speed: 2.5, dir: -1, alive: true,
    ...
    update(dt) {
        ...
        // Camera scrolling
        const scrollTriggerX = this.cameraX + this.vw * CONFIG.SCROLL_THRESHOLD;
        if (p.x > scrollTriggerX && moveRight) {
            const diff = p.x - scrollTriggerX;
            this.cameraX += diff * 0.12;
        }
        
        // Lock backward
        if (this.cameraX > this.maxCameraX + CONFIG.CAMERA_LOCK_DISTANCE) {
            this.maxCameraX = this.cameraX - CONFIG.CAMERA_LOCK_DISTANCE;
        }
        if (this.cameraX < this.maxCameraX) this.cameraX = this.maxCameraX;
            ...
            if (!p.invincible && this.rectsOverlap(p, b)) {
                if (p.vy > 0 && p.y + p.h - 12 < b.y + b.h * 0.4) {
                    ...
                    if (b.health <= 0) {
                        ...
                        this.points += 200;
                        this.spawnParticles(b.x + b.w/2, b.y + b.h/2, '#ffcc00', 20);
        ...
        // Boss spawn check
        if (!this.bossActive && this.points > 0 && this.points >= this.lastBossAt + CONFIG.BOSS_POINTS_INTERVAL) {
            const next = Math.floor(this.points / CONFIG.BOSS_POINTS_INTERVAL) * CONFIG.BOSS_POINTS_INTERVAL;
            if (next > this.lastBossAt) { this.lastBossAt = next; this.spawnBoss(); }
        }
        ...
        for (let i = this.sonicWaves.length - 1; i >= 0; i--) {
            ...
            if (this.bossActive && this.boss && this.boss.alive && this.rectsOverlap(w, this.boss)) {
                this.boss.health--; this.boss.hitFlash = 10;
                ...
                if (this.boss.health <= 0) {
                    ...
                    this.points += 200;
                    this.spawnParticles(this.boss.x + this.boss.w/2, this.boss.y + this.boss.h/2, '#ffcc00', 20);
                    AudioManager.play('victory');
                }
            }
        }
    ...
    render() {
        ...
        // Background (NO blur, NO transparency)
    ...
    // ============ DRAW BACKGROUND (NO blur, NO transparency) ============
    drawBackground(ctx, W, H) {
        // bg_sky: scaled by HEIGHT to 1080px, tiled horizontally, NO FILTER
        const skyImg = ImageCache[ASSETS.bg_sky];
        if (skyImg) {
            const skyH = H;
        ...
        // city_far: scaled by HEIGHT, bottom-aligned, slow parallax, FULL OPACITY, NO BLUR
        const farImg = ImageCache[ASSETS.city_far];
        if (farImg) {
            const farH = H;
        ...
        // city_mid: scaled by HEIGHT, bottom-aligned, medium parallax, FULL OPACITY, NO BLUR
        const midImg = ImageCache[ASSETS.city_mid];
        if (midImg) {
            const midH = H;
    ...
    drawPlatform(ctx, plat) {
        let imgKey;
        if (plat.type === 'main') imgKey = ASSETS.platform_main;
        else if (plat.type === 'static') imgKey = ASSETS.platform_static;
        else imgKey = ASSETS.platform_moving;
        ...
        if (img) {
            ...
            if (plat.type === 'main') {
                // For platform_main: draw FULL tiles, no clipping.
                // Extend the last tile past the platform edge so the PNG is never cut.
                // The platform_main sits at the bottom; visual overflow is hidden below screen.
                ctx.save();
                // No clip — let tiles extend past plat.w so image is never cut mid-tile
                let drawX = plat.x;
                const endX = plat.x + plat.w;
                while (drawX < endX) {
                    // Always draw full tile (even if it overflows past endX)
                    ctx.drawImage(img, drawX, plat.y, drawW, drawH);
                    drawX += drawW;
                }
                ctx.restore();
            } else {
                // For static & moving: tile with partial clip on last tile
                let drawX = plat.x;
                const endX = plat.x + plat.w;
                while (drawX < endX) {
                    ...
                    if (remaining >= drawW) {
                        ...
                        const srcClipW = img.naturalWidth * (remaining / drawW);
                        ctx.drawImage(img, 0, 0, srcClipW, img.naturalHeight, drawX, plat.y, remaining, drawH);
                    }
                    drawX += drawW;
                }
            }
    ...
    drawFlyingEnemy(ctx, fe) {
        const img = ImageCache[ASSETS.flying_enemy];
        if (img) {
    ...
    drawBoss(ctx, b) {
        ...
        if (img) {
            ...
            ctx.fillText(`⚠ BOSS: ${b.name} ⚠`, b.x + b.w/2, barY - 8);
            ctx.shadowBlur = 0;
        }
    },
    ...
    drawSonicWave(ctx, sw) {
        ...
        gradient.addColorStop(0, 'rgba(180, 0, 255, 0.8)');
    ...
    startGame() {
        ...
        this.updateCanvasSize();  // recalculate width for current screen
    ...
    loop(timestamp) {
        ...
        this.render();
        if (this.state === 'playing') UI.updateHUD(this.health, this.energy, this.points);
