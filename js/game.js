...
const Game = {
    ...
    init() {
        ...
        this.setupInput();
        const saved = localStorage.getItem('neorobo_highscore');
    ...
    update(dt) {
        ...
        for (const b of this.bosses) {
            ...
            // Gap detection and floating behavior
            const lookAhead = b.dir > 0 ? b.w + 30 : -30;
            const aheadX = b.x + lookAhead;
            const groundAhead = this.platforms.some(plat => 
                plat.type === 'main' &&
                aheadX >= plat.x && aheadX <= plat.x + plat.w
            );
            
            if (!groundAhead && b.floatState === 'normal') {
                b.floatState = 'rising';
                b.targetY = b.groundY - 50;
            }
            
            if (b.floatState === 'rising') {
                b.y -= 2;
                if (b.y <= b.targetY) {
                    b.y = b.targetY;
                    b.floatState = 'crossing';
                }
            } else if (b.floatState === 'crossing') {
                const centerX = b.x + b.w / 2;
                const groundBelow = this.platforms.some(plat => 
                    plat.type === 'main' &&
                    centerX >= plat.x && centerX <= plat.x + plat.w
                );
                if (groundBelow) {
                    b.floatState = 'descending';
                }
            } else if (b.floatState === 'descending') {
                b.y += 2;
                if (b.y >= b.groundY) {
                    b.y = b.groundY;
                    b.floatState = 'normal';
            ...
            // Movement and facing
            const isBelowPlayer = b.y > p.y;
            const playerMoving = Math.abs(p.vx) > 0.1 || !p.onGround;
            
            if (!isBelowPlayer || playerMoving) {
            ...
            // Sonic wave attack (both mini and main bosses)
            b.sonicTimer += dt;
        ...
        // Boss spawn timers
        if (this.gameTimer - this.lastMiniBossTime >= 40000) {
            this.lastMiniBossTime = this.gameTimer;
            this.spawnBoss(false);
        }
        if (this.gameTimer - this.lastMainBossTime >= 120000) {
            this.lastMainBossTime = this.gameTimer;
            this.spawnBoss(true);
        }
